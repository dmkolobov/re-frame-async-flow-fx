...
(defrecord Rule
	[flow-path id when-fn events dispatch-n halt? capture? seen-events]
		...
		(if halt?
			(conj (if capture?
							(mapv (fn [event-v]
									...
									dispatch-n)
							dispatch-n)
						[:async-flow/halt flow-path (keyword (namespace id))])
			dispatch-n))
...
(defn compile
	[flow-path flow-id index {:keys [id when event events dispatch dispatch-n halt? capture?] :as rule}]
	(map->Rule
